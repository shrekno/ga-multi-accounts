...
def start_proc(command, log_file_path, proc_list):
    ...
    if sys.platform.find('win') == 0:
        proc_list.append(
            {u'proc': subprocess.Popen(command, stderr=f, stdout=f),
             u'status': u'running', u'log': log_file_path, u'command': command})
    elif sys.platform.find('linux') == 0:
        proc_list.append(
            {u'proc': subprocess.Popen(command, stderr=f, stdout=f, close_fds=True),
             u'status': u'running', u'log': log_file_path, u'command': command})
