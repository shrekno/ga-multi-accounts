      ...
      if (u'rows' in result):
        if len(result[u'rows']) == 1 and len(result[u'rows'][0]) == 1:
          ...
          if total_visits <= 5000:
            ...
            summary[u'detail'][u'check_total_visits'] = u'Danger: ' + str(total_visits) + u' in ' + account + u' ' + web_property
          elif total_visits > 8000:
            ret[data[i][u'name']] = u'Error'
            print u'Error:', total_visits, u'in', account, web_property
            summary[u'detail'][u'check_total_visits'] = u'Error: ' + str(total_visits) + u' in ' + account + u' ' + web_property
      else:
  ...
  for i in xrange(0, len(data)):
    ...
    if u'containsSampledData' in result:
      if result[u'containsSampledData']:
        ...
        summary[u'detail'][u'check_sampled'] = u'Error in ' + account + u' ' + web_property
      else:
        ret[data[i][u'name']] = u'Fine'
    else:
  ...
  for func in check_funcs:
    ...
    for account_key in health_status.keys():
      for web_property_key in health_status[account_key].keys():
        data = load_json_data(os.path.join(collect_json_dir, account_key, web_property_key))
        health_status[account_key][web_property_key][func.__name__] = func(data, account_key, web_property_key, summarizing)
        column = health_status[account_key][web_property_key][func.__name__]
