            ...
            if avg_data_flag:
                ...
                if event_data_flag:
                    ...
                    for row in avg_rows:
                        ...
                        row[avg_event_value_index] = str(
                            op(float(row[avg_event_value_index]), float(total_events_map[key_name])))
    ...
    for i in xrange(0, len(left)):
        ...
        if left[i]['name'] == right[i]['name']:
            ...
            if (u'containsSampledData' in l_result) and (u'containsSampledData' in r_result):
                l_result[u'containsSampledData'] = (
                    l_result[u'containsSampledData'] or r_result[u'containsSampledData'])
            ...
            if not u'rows' in r_result:
                ...
                if dimensions_count == 0:
                    for index in xrange(0, len(l_result[u'rows'][0])):
                        metric_data_type = l_result[
                            u'columnHeaders'][index]['dataType']
                        if metric_data_type == u'CURRENCY' or metric_data_type == u'FLOAT' or metric_data_type == u'PERCENT' or metric_data_type == u'US_CURRENCY':
                            l_result[u'rows'][0][index] = str(
                                float(l_result[u'rows'][0][index]) + float(r_result[u'rows'][0][index]))
                        elif metric_data_type == u'INTEGER':
                            l_result[u'rows'][0][index] = str(
                                int(l_result[u'rows'][0][index]) + int(r_result[u'rows'][0][index]))
                        else:  # TIME or other
                            if l_result[u'rows'][0][index] != r_result[u'rows'][0][index]:
                                l_result[u'rows'][0][index] = l_result[u'rows'][
                                    0][index] + u' | ' + r_result[u'rows'][0][index]
                else:
                    dimensions_map = {}
                    ...
                    for dim in r_result[u'rows']:
                        ...
                        if key_name in dimensions_map:
                            for metric_index in xrange(dimensions_count, len(dimensions_map[key_name])):
                                metric_data_type = l_result[
                                    u'columnHeaders'][metric_index]['dataType']
                                if metric_data_type == u'CURRENCY' or metric_data_type == u'FLOAT' or metric_data_type == u'PERCENT' or metric_data_type == u'US_CURRENCY':
                                    dimensions_map[key_name][metric_index] = str(
                                        float(dimensions_map[key_name][metric_index]) + float(dim[metric_index]))
                                elif metric_data_type == u'INTEGER':
                                    dimensions_map[key_name][metric_index] = str(
                                        int(dimensions_map[key_name][metric_index]) + int(dim[metric_index]))
                                else:  # TIME or other
                                    if dimensions_map[key_name][metric_index] != dim[metric_index]:
                                        dimensions_map[key_name][metric_index] = dimensions_map[
                                            key_name][metric_index] + ' | ' + dim[metric_index]
                                        continue
                        else:
...
def main(argv):
    ...
    output_file_path = os.path.join(
        output_dir_path,  profile_id_file_name[:profile_id_file_name.find(u'.txt')] + u'.json')
    ...
    for profile_index in xrange(0, len(profile_id)):
        ...
        init_data_file_path = os.path.join(
            data_dir_path, profile_id[init_data_file_index] + u'.json')
