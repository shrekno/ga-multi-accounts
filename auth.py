    ...
    for cs_file in os.listdir(client_secrets_dir_path):
        if os.path.isfile(os.path.join(client_secrets_dir_path, cs_file)) and cs_file.find(u'.json') > 0:
            token_file_path = os.path.join(
                client_secrets_dir_path, cs_file[:cs_file.find(u'.json')] + u'.dat')
            client_secrets_file_path = os.path.join(
                client_secrets_dir_path, cs_file)
            auth(os.path.abspath(token_file_path),
                 os.path.abspath(client_secrets_file_path), other_param)
